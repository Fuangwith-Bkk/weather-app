...
oc new-project demo-jenkins --display-name "Jenkins"
...
oc new-build --strategy=docker -D $'FROM quay.io/openshift/origin-jenkins-agent-maven:4.1.0\n
   ...
   USER 1001' --name=jenkins-agent-appdev -n demo-jenkins
   
oc get is
oc get pods -w

oc apply -f manifests/jenkins-configmap.yaml -n demo-jenkins
